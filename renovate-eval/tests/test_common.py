    ...
    def test_fallback_rejects_failed_ref_lookup(self, monkeypatch, tmp_dir):
        ...
        def mock_run(cmd, **kw):
            ...
            if "baseRefName" in cmd:
                return subprocess.CompletedProcess(
                    cmd, 1, stdout="", stderr="could not resolve PR"
                )
            ...
            if cmd[:3] == ["git", "diff", "--no-ext-diff"]:
                raise AssertionError("git diff should not run without PR refs")
            return subprocess.CompletedProcess(cmd, 0, stdout=b"", stderr=b"")
        ...
        outfile = os.path.join(tmp_dir, "diff.patch")
        with pytest.raises(RuntimeError, match="Failed to resolve base/head refs"):
            run_diff(1234, outfile)

        assert all(cmd[:3] != ["git", "diff", "--no-ext-diff"] for cmd in commands)
