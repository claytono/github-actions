...
class TestValidateEvalData:
    def test_accepts_any_json_root_type_annotation(self):
        signature = inspect.signature(validate_eval_data, eval_str=True)

        assert signature.parameters["data"].annotation is object
    ...
    def test_optional_fields_reject_whitespace_strings(self, valid_eval_data):
        for field in (
            "performance_stability",
            "features_ux",
            "security",
