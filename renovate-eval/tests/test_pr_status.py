...
class TestGetExistingEvalComment:
    ...
    def test_returns_none_on_nonzero_exit(self, monkeypatch):
        monkeypatch.setattr(
            "lib.pr_status.subprocess.run",
            lambda *a, **kw: subprocess.CompletedProcess(
                a[0], 1, stdout="stale output", stderr="failed"
            ),
        )
        assert get_existing_eval_comment(1234) is None
    ...
    def test_returns_none_on_timeout(self, monkeypatch):
        ...
        monkeypatch.setattr("lib.pr_status.subprocess.run", timeout_run)
    ...
    def test_returns_none_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr("lib.pr_status.subprocess.run", os_error_run)
