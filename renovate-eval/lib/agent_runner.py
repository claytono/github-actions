...
def _raise_agent_timeout(
    *,
    ...
    """Persist captured timeout output and raise a clear provider error."""
    stdout = _stream_text(exc.stdout)
    stderr = _stream_text(exc.stderr)
    if stdout:
        Path(partial_output_path).write_text(stdout)

    timeout_detail = f" after {timeout}s" if timeout is not None else ""
    stderr_detail = f": {stderr[:500]}" if stderr else ""
    raise RuntimeError(
        f"{provider} {role} timed out{timeout_detail}{stderr_detail}"
    ) from exc
...
def _run_claude(
    *,
    ...
    try:
        ...
        _raise_agent_timeout(
            provider="claude",
            role=role,
            timeout=timeout,
            exc=exc,
            partial_output_path=output_json,
        )
