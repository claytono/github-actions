...
def get_repo_root() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Unable to determine git repository root: {exc}") from exc
    repo_root = result.stdout.strip()
    if result.returncode != 0 or not repo_root:
        detail = result.stderr.strip() or result.stdout.strip() or "no output"
        raise RuntimeError(f"Unable to determine git repository root: {detail}")
    return repo_root
...
def _now() -> int:
    """Return monotonic time in seconds for elapsed duration measurements."""
    import time
...
def _post_comment(pr_number: int | str, comment_body: str, artifact_dir: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to find existing renovate-eval comment: {exc}"
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to parse existing renovate-eval comment lookup: {exc}"
        ) from exc
    ...
    if comment_id and str(comment_id) != "null":
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to update existing renovate-eval comment: {exc}"
            ) from exc
    else:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to create renovate-eval comment: {exc}"
            ) from exc
