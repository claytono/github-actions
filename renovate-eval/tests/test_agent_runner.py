...
def test_run_claude_timeout_writes_partial_output(monkeypatch, tmp_dir):
    ...
    monkeypatch.setattr("lib.agent_runner.subprocess.run", timeout_run)
    output_json = os.path.join(tmp_dir, "claude-output.json")

    with pytest.raises(RuntimeError, match="claude evaluator timed out after 600s"):
        run_agent(
            provider="claude",
            role="evaluator",
            prompt="prompt",
...
def test_run_codex_timeout_writes_partial_jsonl(monkeypatch, tmp_dir):
    raw = json.dumps({"type": "thread.started", "thread_id": "thread-123"}) + "\n"
    ...
    with pytest.raises(RuntimeError, match="codex evaluator timed out after 600s"):
        run_agent(
            provider="codex",
            role="evaluator",
            prompt="prompt",
